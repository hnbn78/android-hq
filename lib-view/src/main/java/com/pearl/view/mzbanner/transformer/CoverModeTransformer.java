package com.pearl.view.mzbanner.transformer;
...
public class CoverModeTransformer implements ViewPager.PageTransformer {
    ...
    public void transformPage(View view, float position) {
        View shadowImage = ((ViewGroup) view).findViewById(R.id.mz_banner_shadow_tag);
        if (mViewPager.getAdapter() != null && mViewPager.getAdapter().getCount() < 3) {
            shadowImage.setVisibility(View.INVISIBLE);
        }else{
            shadowImage.setVisibility(View.VISIBLE);
        }
        View imgImage = ((ViewGroup) view).findViewById(R.id.mz_banner_imag_tag);
        ...
        if (shadowImage != null) {
            shadowImage.setAlpha(1 - (view.getScaleX() - mScaleMin)/(1 - mScaleMin));
        }
    }
}
