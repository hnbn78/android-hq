...
public class LockColumnAdapter extends RecyclerView.Adapter<LockColumnAdapter.LockViewHolder> {
    ...
    @Override
    public void onBindViewHolder(final LockViewHolder holder, final int position) {
        //设置布局
        ...
        if (!isLockFristRow) {
            if (position == 0) {
                ...
                if(mTableFirstRowTextColor==0){
                    holder.mTextView.setTextColor(ContextCompat.getColor(mContext, mTableHeadTextColor));
                }else{
                    holder.mTextView.setTextColor(ContextCompat.getColor(mContext, mTableFirstRowTextColor));
                }
        ...
        if(mOnCellLongClickListener!=null){
            holder.mTextView.setOnLongClickListener(new View.OnLongClickListener() {
                @Override
                public boolean onLongClick(View v) {
                    holder.mTextView.setTag(mLockColumnDatas.get(position));
                    mOnCellLongClickListener.onCellLongClick(v, position,0, mLockColumnDatas.get(position));
