 ...
 */
public class PlayBoardLHCView extends LinearLayout {
    ...
    //启用的组
    private ArrayList<View> listNumBallGroup = new ArrayList<>();
    ...
    public PlayBoardLHCView(@NonNull Context context) {
        super(context);
    }

    public PlayBoardLHCView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public PlayBoardLHCView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setConfig(List<LotteryPlayLHCUI.CellUIBean> layoutBeans, LotteryPlayUserInfoLHC userInfoLHC) {
        ...
        for (int i = 0; i < layoutBeans.size(); i++) {
            ...
            if (bean.getShowColors().size() > 0 && bean.getShowColors().get(0) != null && bean.getShowColors().get(0) > 17) {
                PlayBoardWeishuLHCItemView view = new PlayBoardWeishuLHCItemView(getContext());
                view.setUIConfig(bean, userInfoLHC);
                addView(view);
                view.setTag(i);
                listNumBallGroup.add(view);
            } else {
                PlayBoardLHCItemView view = new PlayBoardLHCItemView(getContext());
                view.setUIConfig(bean, userInfoLHC);
                addView(view);
                view.setTag(i);
                listNumBallGroup.add(view);
