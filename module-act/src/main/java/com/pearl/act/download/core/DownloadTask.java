...
import com.pearl.act.download.utils.FileUtils;
...
public class DownloadTask implements ConnectThread.ConnectListener, DownloadThread.DownloadListener {
    ...
    private long subThreadRefreshInterval = 15000;
    ...
    private void startMultiThreadDownload() {
        ...
        for (int i = 0; i < DownloadConfig.getInstance().getMax_download_threads(); i++) {
            ...
            if (startPos < endPos) {
                downloadThreads[i] = new DownloadThread(entry.url, entry.name, i, startPos, endPos, this);
    ...
    private void startSingleThreadDownload() {
        ...
        downloadThreads[0] = new DownloadThread(entry.url, entry.name, 0, 0, 0, this);
