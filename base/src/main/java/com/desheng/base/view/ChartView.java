...
public class ChartView extends View {
    ...
    //x轴坐标对应的数据
    private List<String> xCoordinate = new ArrayList<>();
    //y轴坐标对应的数据
    private List<Float> yCoordinate = new ArrayList<>();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            height = getHeight();
            //Y轴文本最大宽度
            float textYWdith = getTextBounds("0000000000", xyTextPaint).width();
            for (int i = 0; i < yCoordinate.size(); i++) {//求取y轴文本最大的宽度
                float temp = getTextBounds(yCoordinate.get(i) + "", xyTextPaint).width();
                if (temp > textYWdith)
                    textYWdith = temp;
            }
            int dp2 = dpToPx(2);
...
//            //X轴文本最大高度
            ...
            for (int i = 0; i < xCoordinate.size(); i++) {//求取x轴文本最大的高度
                Rect rect = getTextBounds(xCoordinate.get(i) + "", xyTextPaint);
            ...
            xInit = interval + xOri;
            minXInit = width - (width - xOri) * 0.1f - interval * (xCoordinate.size() - 1);//减去0.1f是因为最后一个X周刻度距离右边的长度为X轴可见长度的10%
            maxXInit = xInit;
        }
...
//        super.onDraw(canvas);
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        for (int i = 0; i < xCoordinate.size(); i++) {
            ...
            float y = yOri - yOri * (1 - 0.1f) * value.get(xCoordinate.get(i)) / yCoordinate.get(yCoordinate.size() - 1);
            ...
            if (i == selectIndex - 1) {
                ...
                drawFloatTextBox(canvas, x, y - dp7, value.get(xCoordinate.get(i)));
            }
            //绘制普通的节点
            linePaint.setStyle(Paint.Style.FILL);
...
//        canvas.drawPath(path, linePaint);
        ...
        Rect rect = getTextBounds( "0000000000", linePaint);
    ...
    private void drawBrokenLine(Canvas canvas) {
        ...
        float y = yOri - yOri * (1 - 0.1f) * value.get(xCoordinate.get(0)) /Float.valueOf( yCoordinate.get(yCoordinate.size() - 1));
        path.moveTo(x, y);
        for (int i = 1; i < xCoordinate.size(); i++) {
            x = xInit + interval * i;
            y = yOri - yOri * (1 - 0.1f) * value.get(xCoordinate.get(i)) / Float.valueOf(yCoordinate.get(yCoordinate.size() - 1));
            path.lineTo(x, y);
        }
        canvas.drawPath(path, linePaint);
    }
    ...
    private void drawXY(Canvas canvas) {
        ...
        //绘制y轴刻度
        int yLength = (int) (yOri * (1 - 0.1f) / (yCoordinate.size() - 1));//y轴上面空出10%,计算出y轴刻度间距
        ...
        path = new Path();
        //整个X轴的长度
        float xLength = xInit + interval * (xCoordinate.size() - 1) + (width - xOri) * 0.1f;
        if (xLength < width)
            xLength = width;
            ...
            if (x >= xOri) {//只绘制从原点开始的区域
                ...
                if (i == selectIndex - 1) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        canvas.drawRoundRect(x - xValueRect.width() / 2 - dpToPx(3), yOri + xylinewidth + dpToPx(1),
                                x + xValueRect.width() / 2 + dpToPx(3),
                                yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2),
                                dpToPx(2), dpToPx(2), xyTextPaint);
                    }
                } else {
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + rect.height(), xyTextPaint);
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (interval * xCoordinate.size() > width - xOri) {//当期的宽度不足以呈现全部数据
    ...
    private void clickAction(MotionEvent event) {
        ...
        for (int i = 0; i < xCoordinate.size(); i++) {
            //节点
            float x = xInit + interval * i;
            float y = yOri - yOri * (1 - 0.1f) * value.get(xCoordinate.get(i)) / Float.valueOf(yCoordinate.get(yCoordinate.size() - 1));
