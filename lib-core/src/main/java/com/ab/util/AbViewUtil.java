...
public class AbViewUtil {
    ...
    */
    ...
    public static void setViewHeightByRatio(View view, float ratio){
        int widthPixels = getViewWidth(view);
        int heightPixels = getViewHeightByRatio(view, ratio);
        //不能这样, 转型错误
        //ViewGroup.LayoutParams params = new LayoutParams(width, height);
        ViewGroup.LayoutParams params = view.getLayoutParams();
        if(params == null){
            AbDebug.log(TAG, "setViewHeightByRatio出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
        if(ratio == 0){
            AbDebug.log(TAG, "setViewHeightByRatio出错,ratio为0");
            return;
        }
        if (widthPixels != INVALID){
     ...
     * @return
     */
    public static int getViewHeightByRatio(View view, float ratio){
        int height = Math.round(getViewWidth(view)*ratio);
        return height;
    }
    
    public static void setViewHeightByRatio(View view, int addition, float ratio){
        int widthPixels = getViewWidth(view);
        int heightPixels = getViewHeightByRatio(view, ratio);
        //不能这样, 转型错误
        //ViewGroup.LayoutParams params = new LayoutParams(width, height);
        ...
        if (heightPixels != INVALID){
            params.height = heightPixels + addition;
        }
        view.setLayoutParams(params);
    }
