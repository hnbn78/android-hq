...
public class AbSlidingPlayView extends LinearLayout {
	public static final int NAV_STYLE_DOTS = 1;
	public static final int NAV_STYLE_NUMBERS = 2;
	...
	/** 导航图片. */
	private Bitmap displayImage = null, hideImage = null;
	/** 导航图片ID. */
	private int displayImageId = -1, hideImageId = -1;
	...
	/**导航显示方式**/  
	private int iNavStyle = 1;
	
	/**使用数字导航时的样式**/
	private int iNavTextSize = 14;
	private int idNavTextColor = 14;
	...
	public void initView(Context context){
		...
		mViewPager.setId(R.id.ab_sliding_view_pager);
		//导航的点
	...
	/**
	 * 设置导航指示样式. 在addView前调用.
	 * TODO.
	 */
	public void setNavDotStyle() {
		this.iNavStyle = NAV_STYLE_DOTS;
	}
	 ...
	 * TODO.
	 */
	public void setNavNumberStyle(int textSize, int textColorId) {
		this.iNavStyle = NAV_STYLE_NUMBERS;
		iNavTextSize = textSize;
		idNavTextColor = textColorId;
	}
	...
	public void creatIndex() {
		...
		if(iNavStyle == NAV_STYLE_DOTS){
			for (int j = 0; j < count; j++) {
				ImageView imageView = new ImageView(context);
				imageView.setLayoutParams(navLayoutParams);
				if (j == 0) {
					if(displayImageId != -1){
						imageView.setImageResource(displayImageId);
					}else if(displayImage != null){
						imageView.setImageBitmap(displayImage);
					}
				} else {
					if(hideImageId != -1) {
						imageView.setImageResource(hideImageId);
					} else if (hideImage != null) {
		...
		} else if(iNavStyle == NAV_STYLE_NUMBERS) {
			TextView current = new TextView(context);
			current.setText(""+ 1);
			current.setTextColor(context.getResources().getColor(idNavTextColor));
			current.setTextSize(iNavTextSize);
			current.setLayoutParams(navLayoutParams);
			navLinearLayout.addView(current, 0);
			
			TextView total = new TextView(context);
			total.setText("/" + count);
			total.setTextColor(context.getResources().getColor(idNavTextColor));
			total.setTextSize(iNavTextSize);
			total.setLayoutParams(navLayoutParams);
			navLinearLayout.addView(total, 1);
		}
	...
	public void makesurePosition() {
		...
		if(iNavStyle == NAV_STYLE_DOTS){
			for (int j = 0; j < count; j++) {
				if (position == j) {
					if(displayImageId != -1){
						((ImageView)navLinearLayout.getChildAt(position)).setImageResource(displayImageId);
					} else if(displayImage != null){
						((ImageView)navLinearLayout.getChildAt(position)).setImageBitmap(displayImage);
					}
				} else {
					if(displayImageId != -1){
						((ImageView)navLinearLayout.getChildAt(j)).setImageResource(hideImageId);
					} else if(displayImage != null){
			...
			((TextView)navLinearLayout.getChildAt(0)).setText("" + (1 + position));
			((TextView)navLinearLayout.getChildAt(1)).setText("/" + count);
		}
	 ...
	 * @param hideImage the hide image
	 */
	public void setPageLineImage(int displayResId,int hideResId) {
		this.displayImageId = displayResId;
		this.hideImageId = hideResId;
		creatIndex();
	}
