package com.pearl.view.rmondjone.locktableview;
...
public class UnLockColumnAdapter extends RecyclerView.Adapter<UnLockColumnAdapter.UnLockViewHolder> {
     ...
     * 表格内容字体颜色
     */
    private String[] mTableContentTextColumnColors;
    private LockTableView.OnCellClickListenter onCellClickListenter;

    private LockTableView.OnCellLongClickListener mOnCellLongClickListener;
    ...
    public void setTableContentTextColumnColors(String[] mTableContentTextColors) {
        this.mTableContentTextColumnColors = mTableContentTextColors;
    }
    ...
    public void setOnCellClickListenter(LockTableView.OnCellClickListenter onCellClickListenter) {
        this.onCellClickListenter = onCellClickListenter;
    }

    public LockTableView.OnCellClickListenter getOnCellClickListenter() {
        return onCellClickListenter;
    }
     ...
     */
    private void createRowView(LinearLayout linearLayout, final List<String> datas, boolean isFristRow, int mMaxHeight, final int position) {
        //设置LinearLayout
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            if (datas.get(i).contains("@")) {
                ...
                final String[] actions = datas.get(i).split("@");
                String[] colors = {"#2095F2", "#FF0000", "#FF00FF00", "#0066CC", "#FFFFFF00","#169390","#ff00ff"};

                LinearLayout actionLayout = new LinearLayout(mContext);
                actionLayout.setOrientation(LinearLayout.HORIZONTAL);

                for (int k = 0; k < actions.length; k++) {
                    final TextView textView = new TextView(mContext);
                    textView.setText(actions[k]);
                    textView.setTextColor(Color.parseColor(colors[k]));

                    setActionTextStyle(mMaxHeight, i, textView, actions.length);

                    if (onCellClickListenter != null) {
                        textView.setTag(k);
                        textView.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                onCellClickListenter.onCellClick(v, position, (Integer) textView.getTag(), actions[(Integer) textView.getTag()]);
                            }
                        });
                    }
                ...
                linearLayout.addView(actionLayout);
                ...
                if (isFristRow) {
                    ...
                    if (mTableContentTextColumnColors != null && !Strs.isEmpty(mTableContentTextColumnColors[i])) {
                        textView.setTextColor(Color.parseColor(mTableContentTextColumnColors[i]));
                    } else {
                ...
                if (onCellClickListenter != null) {
                    ...
                    textView.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            onCellClickListenter.onCellClick(v, position, (Integer) textView.getTag(), datas.get((Integer) textView.getTag()));
                        }
                    });
                }

                if(mOnCellLongClickListener !=null && i==2){
                    textView.setTag(i);
                    textView.setOnLongClickListener(new View.OnLongClickListener() {
                        @Override
                        public boolean onLongClick(View v) {
                            mOnCellLongClickListener.onCellLongClick(v, position, (Integer) textView.getTag(), datas.get((Integer) textView.getTag()));
                            return false;
                        }
                    });
                }
    ...
    private void setActionTextStyle(int mMaxHeight, int i, TextView textView, int count) {
        textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTextViewSize);
        textView.setGravity(Gravity.CENTER);
        LinearLayout.LayoutParams textViewParams = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
                LinearLayout.LayoutParams.WRAP_CONTENT);
        ...
        textViewParams.height = DisplayUtil.dip2px(mContext, mMaxHeight);
        textViewParams.width = DisplayUtil.dip2px(mContext, mColumnMaxWidths.get(i + 1) / count);
        textView.setLayoutParams(textViewParams);
    }

    public void setOnCellLongClickListener(LockTableView.OnCellLongClickListener mOnCellLongClickListener){
        this.mOnCellLongClickListener=mOnCellLongClickListener;
    }
}
