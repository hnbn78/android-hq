...
import com.desheng.base.databinding.ActLotteryOpenHkBinding;
 ...
 */
public class ActLotteryOpenHC extends AbAdvanceActivity<ActLotteryOpenHkBinding> implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public static void launch(Activity activity, int lotteryId) {
        Intent itt = new Intent(activity, ActLotteryOpenHC.class);
    ...
    protected int getLayoutId() {
        return R.layout.act_lottery_open_hk;
    }
    ...
    private int[] ball_bgs = {R.drawable.sh_bd_circle_red_ball, R.drawable.sh_bd_circle_blue_ball, R.drawable.sh_bd_circle_green_ball};
    ...
    protected class OpenAdapter extends BaseQuickAdapter<LotteryOpenHistory, OpenAdapter.OpenVH> {
        ...
        public OpenAdapter(Context ctx) {
            super(R.layout.item_lottery_open_hk, new ArrayList<LotteryOpenHistory>());
            this.ctx = ctx;
        }

        @Override
        protected void convert(OpenVH holder, LotteryOpenHistory item) {
            String text = String.valueOf(Dates.getStringByFormat(item.getTime() * 1000, Dates.dateFormatYMD));
            holder.tvIssue.setText("第" + item.getIssue() + "期");

            if (Strs.isNotEmpty(item.getCode())) {
                holder.layout_balls.removeAllViews();
                String[] codes = item.getCode().split(",");
                for (String code : codes) {
                    View view = LayoutInflater.from(ctx).inflate(R.layout.view_circle_code_ball, null);
                    TextView tv_code = view.findViewById(R.id.tv_open_code);
                    tv_code.setText(code);
                    int codein = Integer.parseInt(code);
                    tv_code.setBackgroundResource(ball_bgs[codein % 3]);

                    holder.layout_balls.addView(view);
                }
        ...
        protected class OpenVH extends BaseViewHolder {
            ...
            public LinearLayout layout_balls;

            public OpenVH(View view) {
                super(view);
                tvIssue = view.findViewById(R.id.tvIssue);
                layout_balls = view.findViewById(R.id.layout_balls);
            }
        }
    }
}
