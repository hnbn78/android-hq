...
import com.desheng.app.toucai.panel.FragLotteryPlayFragmentBoardJD;
 ...
 */
public class CtrlPlay_Board_JD extends BaseCtrlPlayJD implements PlayBoardJDView.OnBallsChangeListener {

    ArrayList<PlayBoardJDView> listBallGroup;
    ...
    public CtrlPlay_Board_JD(Context ctx, FragLotteryPlayFragmentBoardJD frag, LotteryInfo info, LotteryPlayJD uiConfig) {
    ...
    public void setContentGroup() {
        ...
        listBallGroup = frag.getListBoardGroupsJD();
        ...
        for (int i = 0; i < listBallGroup.size(); i++) {
            //初始化控件
            PlayBoardJDView lotteryPlayJDView = listBallGroup.get(i);
            lotteryPlayJDView.setConfig(getUiConfig().getLayout().get(i), frag.getUserPlayInfoJD(), frag.getMode());
            lotteryPlayJDView.setBallMultiSelection();
            lotteryPlayJDView.setOnBallsChangeListener(this);

            //初始化各组数据
            arrAllBallsSelected[i] = lotteryPlayJDView.getArrBallSelected();
            arrAllBalls[i] = ArraysAndLists.toStringArray(lotteryPlayJDView.getListBalls());
            arrAllNums[i] = ArraysAndLists.toStringArray(lotteryPlayJDView.getListNum());
            arrAllTitles[i] = lotteryPlayJDView.getTitle();
            arrAllIds[i] = lotteryPlayJDView.getArrIds();
    ...
    public void autoGenerate() {
        ...
        int i = random.nextInt(arrAllBallsSelected.length);
        //先滚到顶
                ...
                mDisposable = Flowable.interval(400, TimeUnit.MILLISECONDS)
                        ...
                        .subscribe(new Consumer<Long>() {
                            ...
                            public void accept(@NonNull Long aLong) throws Exception {
                                ...
                                final PlayBoardJDView view = listBallGroup.get(i);
        ...
        for (int i = 0; i < arrAllBallsSelected.length; i++) {
            for (int j = 0; j < arrAllBallsSelected[i].length; j++) {
                if (arrAllBallsSelected[i][j]) {
                    ...
                    String rate = userPlayInfo.getObj().getLines().get(getUiConfig().getLayout().get(i).getBalls().get(j));
